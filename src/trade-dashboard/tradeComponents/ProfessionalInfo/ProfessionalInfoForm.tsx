...
const ProfessionalInfoForm = () => {
  ...
  useEffect(() => {
    if (saved) {
      ...
      if (saved.yearsExperience) setValue('yearsExperience', saved.yearsExperience);
  ...
  <div className="mb-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
        ...
        <div>
          ...
          <select {...register("businessType")} className="w-full border border-gray-200 rounded-lg px-4 py-2 focus:ring-2 focus:ring-orange-500 focus:outline-none bg-[#F8F9FA]">
          ...
          {categories.slice(0, 6).map((category: any) => {
            ...
            return (
              <div
                ...
                className={`border rounded-lg px-4 py-2 text-center cursor-pointer hover:bg-gray-100 flex flex-col items-center gap-2 ${isSelected ? 'border-orange-400 bg-orange-50 ring-2 ring-orange-200' : 'border-gray-200'}`}
              >
                {category.image && (
                  <img
                    src={category.image}
      ...
      <div className="py-10">
        ...
        <div className="grid grid-cols-3 gap-5">
          {subCategories.length === 0 && (
            <div className="text-sm text-gray-500">Select a category to see its subcategories</div>
          )}
          {subCategories.map((sub: string | { id?: string; name?: string; title?: string }, idx: number) => {
            const id = typeof sub === 'string' ? sub : sub?.id ?? String(idx);
            const label = typeof sub === 'string' ? sub : sub?.name ?? sub?.title ?? String(sub);
            ...
            return (
              <div
                ...
                className={`border rounded-lg px-4 py-2 text-center cursor-pointer hover:bg-gray-100 ${isSelected ? 'border-orange-400 bg-orange-50' : 'border-gray-200'}`}
              >
